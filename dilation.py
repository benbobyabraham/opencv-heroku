...
def main_image():
    ...
    if uploaded_file is not None:
        ...
        grayscale_image = anant.grayscale(image)
        thresh_value = st.slider("SET THE THRESHOLD VALUE", min_value = 20,  max_value = 200)
        threshold_image = anant.thresholding(grayscale_image, thresh_value,cv2.THRESH_BINARY_INV)
        eroded_image = anant.dilation(threshold_image, )
        with c2:
...
def main_video(type_):
    ...
    if video is not None:   
        while True:
            ...
            threshold_frame = anant.thresholding(grayscale_frame, attr = cv2.THRESH_BINARY_INV)
            eroded_frame = anant.dilation(threshold_frame)
            placehodler_2.image(eroded_frame)
