...
def main_image():
    ...
    if uploaded_file is not None:
        ...
        with c1:
            st.image(image)
        image = anant.grayscale(image)
        thresh_value = st.slider("SET THE THRESHOLD VALUE", min_value = 20,  max_value = 200)
        threshold_image = anant.thresholding(image,thresh_value)
        with c2:
...
def main_video(type_):
    ...
    if video is not None:   
        while True:
            ...
            threshold_frame = anant.thresholding(grayscale_frame)
            placehodler_2.image(threshold_frame)
